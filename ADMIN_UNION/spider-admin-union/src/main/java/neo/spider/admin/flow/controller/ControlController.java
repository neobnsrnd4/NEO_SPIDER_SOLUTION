...
public class ControlController {
	...
	public ControlController(ControlService controlService, MessagePublisher messagePublisher,
			ObjectMapper objectMapper, BulkheadService bulkheadService, RateLimiterService rateLimiterService,
	...
	@GetMapping("/list")
	public String findPage(Model model, @ModelAttribute SearchDto dto, @RequestParam(defaultValue = "1") int page,
	...
	public String detail(Model model, @PathVariable("id") long id, @RequestParam(required = false) String bulkheadUrl,

			@RequestParam(required = false, defaultValue = "-1") int ratelimiterType,
			@RequestParam(required = false) String ratelimiterUrl,

			@RequestParam(required = false, defaultValue = "-1") int ratelimiterTypeByRedis,
			@RequestParam(required = false) String ratelimiterUrlByRedis
		...
		List<RateLimiterSearchDto> rateLimiters = rateLimiterService.findByApplication(application.getApplicationId(),
				ratelimiterType, ratelimiterUrl);

		List<RateLimiterSearchDto> rateLimitersByRedis = rateLimiterRedisService
				.findByApplication(application.getApplicationId(), ratelimiterTypeByRedis, ratelimiterUrlByRedis);
		...
		model.addAttribute("rateLimitersByRedis", rateLimitersByRedis);
		model.addAttribute("ratelimiterTypeByRedis", ratelimiterTypeByRedis);
		model.addAttribute("ratelimiterUrlByRedis", ratelimiterUrlByRedis);
